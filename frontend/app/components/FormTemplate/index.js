...
import ImageTemplate from "../ImageTemplate/index";
...
const FormTemplate = (props) => {
  ...
  const changeHandler = (newValue, input) => {
    setFormValues((prev) => {
      ...
      const orderedArray = resultArray.sort((a, b) =>
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.inputs}>
        {formValues.map((input) => renderInput(input))}
      </ScrollView>
      <View style={{ marginTop: 30 }}>
        {props.changeHobbies ? (
          <Button
            title="Modify your hobbies"
            onPress={() => navigation.navigate("EditHobbiesScreen")}
          />
        ) : null}

        <CustomButton
          text="Submit"
          handlePress={() => props.onSubmit(formValues)}
        />
      </View>
    </View>
  );
